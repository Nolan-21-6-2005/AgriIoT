...
def _render_ai_decision(devices):
    """Render the rule-based irrigation recommendation above the simulator."""
    with st.container(border=True, key="ai_decision_card"):
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("#### Khuyến nghị tưới")
        with col2:
            with st.container(horizontal_alignment="right", key = "decision"):
                decision_btn = st.button("Kiểm tra", type="primary", icon=":material/water_drop:", key="map_ai_predict")
        st.caption("Hệ thống áp dụng các điều kiện xác định trước để khuyến nghị tưới; phần mô phỏng bên dưới mới thực hiện lệnh cho máy bơm.")
        cols = st.columns(5)
        ...
        with result_slot.container():
            if result:
                if result.get("irrigation"):
                    st.warning("KHUYẾN NGHỊ: CẦN TƯỚI")
                else:
                    st.success("KHUYẾN NGHỊ: KHÔNG CẦN TƯỚI")
                st.caption(f"Phương pháp: {result.get('model', '-')}")
            else:
...
def _render_irrigation_simulator(devices, values):
    with st.container(border=True, key="irrigation_simulator_card"):
        ...
        with col2:
            with st.container(horizontal_alignment="right", key = "irrigation"):
                irrigation_btn = st.button("Chạy lệnh tưới", type="primary", icon=":material/water_drop:", key="map_execute_irrigation")
...
def show_garden_map():
    ...
    left, right = st.columns([3.2, 2.5], gap="small")
    with left:
